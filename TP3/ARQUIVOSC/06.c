...
void initPilhaShow(PilhaShow* pilha, int initialCapacity) {
    pilha->array = (Show*)malloc(initialCapacity * sizeof(Show));
    if (pilha->array == NULL) {
        fprintf(stderr, "Memory allocation failed for PilhaShow array\n");
        exit(EXIT_FAILURE);
    }
    pilha->topo = -1; // Stack is empty
    pilha->capacity = initialCapacity;
}

// Function to free memory used by PilhaShow
void freePilhaShow(PilhaShow* pilha) {
    // Iterate from the current top down to 0 to free each Show's dynamic members
...
// Empilhar (Push): Inserts a show onto the top of the stack with dynamic resizing
void empilhar(PilhaShow* pilha, Show show) {
    // Check if capacity needs to be increased
    if (pilha->topo == pilha->capacity - 1) {
        pilha->capacity *= 2; // Double the capacity
        pilha->array = (Show*)realloc(pilha->array, pilha->capacity * sizeof(Show));
        if (pilha->array == NULL) {
            fprintf(stderr, "Memory reallocation failed for PilhaShow - cannot push.\n");
...
Show desempilhar(PilhaShow* pilha) {
    ...
    // It's the caller's responsibility to free its dynamic members.
    return resp;
}

// Mostrar: Displays all shows in the stack from top to bottom
void mostrarPilha(const PilhaShow* pilha) {
    // Iterate from topo down to 0 to print in LIFO order
    for (int i = pilha->topo; i >= 0; i--) {
...
int main() {
    ...
    PilhaShow pilha;
    // Start with a reasonable initial capacity. It will grow as needed.
    initPilhaShow(&pilha, 100); 
    ...
    for (int i = 0; i < n_commands; i++) {
        ...
        if (strcmp(cmd, "I") == 0) { // Push command
            ...
            fprintf(stderr, "Warning: Unrecognized command '%s'. Only 'I <id>' and 'R' are supported for a stack.\n", cmd);
        }
    }
